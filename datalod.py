...
def vectorized_label(target, n_class):
    ...
    #dict = {'TCF3-PBX1': 0, 'ETV6-RUNX1': 1, 'PAX5': 2, 'CRLF2-P2RY8': 3, 'KMT2A': 4, 'BCR-ABL': 5, 'ZNF384': 6,
            #'MEF2D': 7,
            #'hyper>49 or 50': 8, 'DUX4': 9}
    TARGET = np.array(target).reshape(-1) # x
...
def load_data(path, dtype):
    ...
    x = data.drop(["LTS_LABEL"], axis=1)  # x
    #x.drop(columns=x.columns[0], inplace=True)  # x don't know why extra first column
    x = x.values  # x
    y = data.loc[:, ["LTS_LABEL"]].values  # x
    X = torch.from_numpy(x).type(dtype)
    Y = torch.from_numpy(vectorized_label(y, 2)).type(dtype)  # Changed if number classes changes
